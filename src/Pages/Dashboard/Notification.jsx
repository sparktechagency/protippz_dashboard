  ...
  return (
    <div className="bg-[var(--bg-white-20)] p-4 rounded-md">
      ...
      <div className="start-start gap-2 flex-col">
        {data?.data?.result?.map((item, i) => (
          <div
            ...
            className={`grid-cols-7 w-full grid gap-4 card-shadow rounded-md p-2 cursor-pointer ${
              item?.isRead ? 'bg-[var(--bg-white)]' : 'bg-[var(--bg-gray-20)]'
            }`}
            key={i}
          >
      ...
      <Button
        style={{ padding: '10px' }}
        text={'Load More'}
        classNames="button-blue w-full mt-5 "
        handler={() => setLimit(limit + 30)}
      ></Button>
    </div>
  );
};
